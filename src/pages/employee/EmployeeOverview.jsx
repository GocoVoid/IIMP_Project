...
const EmployeeOverview = () => {
  ...
  const {
    allTickets,       // ← unfiltered, for accurate recent slice + count
    stats,
    loading,
    fetchAllByUser,   // ← fetch on mount
    createTicket,
    updateStatus,
    assignTicket,
    addComment,
  ...
  /* ✅ Sort by newest first, then take top 5 from UNFILTERED list */
  ...
  return (
    <DashboardLayout title="Overview">
      <div className="space-y-6 animate-fade-in">
        ...
        {/* Stats — from getIncidentStatsByUser */}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm overflow-hidden">
          ...
          /* List */
            ...
            <>
              <ul className="divide-y divide-gray-50">
                {recentTickets.map((t) => (
                  ...
                  >
                    {/* ID */}
                    <span
                      className="font-mono text-xs font-medium w-36 shrink-0 group-hover:text-indigo-700 transition-colors"
                      style={{ color: '#3c3c8c' }}
                    >
                      {t.id}
                    </span>
              ...
              {/* Footer — only when there are more than 5 */}
