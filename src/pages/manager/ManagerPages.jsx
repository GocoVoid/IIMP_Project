...
export const ManagerTickets = () => {
  ...
  /* Derive unique departments and categories from full ticket list */
        ...
        {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={refetch} /> : (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm overflow-hidden">
            <div className="px-5 py-4 flex flex-wrap items-center gap-3" style={{ borderBottom:'1px solid #f3f4f6' }}>
              ...
              {/* <select value={filters.department ?? ''} onChange={e => updateFilter('department', e.target.value)} className={selCls}>
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              {/* <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm p-5 space-y-4">
                ...
                <p className="text-xs text-gray-500 -mt-2">Select a ticket above, then post a comment.</p>

                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1.5">Ticket</label>
                  <div className="px-3 py-2.5 text-sm rounded-xl border border-gray-100 bg-gray-50 text-gray-500">
                    {selectedTicket ? selectedTicket : <span className="text-gray-400 italic">No ticket selected</span>}
                  </div>
                </div>
                ...
                <form onSubmit={handleComment} className="space-y-3">
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-3 py-2 text-sm rounded-xl border border-gray-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none resize-none"
                    />
                  </div>
                  ...
                  <button type="submit"
                    disabled={!selectedTicket || !comment.trim() || commenting}
                    className="w-full py-2.5 rounded-xl text-sm font-medium text-white disabled:opacity-50 transition-colors"
                    style={{ background: 'linear-gradient(135deg,#3c3c8c,#4f4fa3)' }}>
                ...
                {commentSuccess && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-green-50 border border-green-200 animate-fade-in">
                    ...
                    <p className="text-xs text-green-700">{commentSuccess}</p>
                  </div>
                )}
                {commentError && <p className="text-xs text-red-500 px-1">{commentError}</p>}
              </div> */}
            </div>

            {/* Unassigned open tickets list */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm overflow-hidden">
              <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #f3f4f6' }}>
                <div>
...
export const ManagerReports = () => {
  ...
  /* Daily breakdown is always fetched separately at 'day' granularity */
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        {reportLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            {/* Volume Trend — respects the selected period (Weekly/Monthly/Yearly) */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm p-5">
            ...
            {/* <div className="bg-white rounded-2xl border border-gray-100 shadow-pratiti-sm p-5 lg:col-span-2">
              ...
              {dailyLoading ? (
                ...
                <p className="text-xs text-gray-400 text-center py-10">No daily data available.</p>
              ) : (
                <div className="flex items-end gap-1.5 h-36">
                  {dailyVolume.map(d => (
                    <div key={d.label} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                      <span className="text-[10px] text-gray-500">{d.count}</span>
                      <div className="w-full rounded-t-lg transition-all"
                        style={{ height:`${Math.max((d.count / maxDaily) * 112, d.count > 0 ? 4 : 0)}px`, background:'linear-gradient(to top,#14a0c8,#2db8e0)' }} />
                      <span className="text-[10px] text-gray-400 truncate w-full text-center">{d.label}</span>
                    </div>
                  ))}
                </div>
...
export const ManagerMyTickets = () => {
  ...
  /* Filter to tickets created by current user */
  ...
  /* Stats derived from unfiltered my-tickets */
  const myStats = {
